...
public:
    ...
    {
        memcpy(m_counter, &counter, sizeof(m_counter));
    }
    ...
    {
        memcpy(m_counter, other.m_counter, sizeof(m_counter));
    }
    ...
    //! \name Operators
    //@{
    const AESCounter<S> & operator = (const AESCounter<S> & counter) { setCounter(counter); return *this; }
    const AESCounter<S> & operator = (const counter_t & counter) { setCounter(counter); return *this; }
    const AESCounter<S> & operator = (const counter_t * counter) { setCounter(counter); return *this; }
