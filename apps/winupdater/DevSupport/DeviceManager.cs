...
//    }
    ...
    /// </summary>
    public sealed class DeviceManager : NativeMethods.IQueryCancelAutoPlay, NativeMethods.IHWEventHandler, IDisposable
    {
        // private static SynchronizationContext _SynchronizationContext;
        private static DispatcherSynchronizationContext _SynchronizationContext = new DispatcherSynchronizationContext();
        private static object _TheLock;
        private static DeviceChangeWindow _DevChangeWnd;
        public static Collection<DeviceClass> DeviceClassMgrs { get; private set; }
//        private static MSWMDMLib.IWMDeviceManager _WMDMDevMgr = new MSWMDMLib.IWMDeviceManager();
        ...
        static DeviceManager()
        {
            try
            {
                ...
                DeviceClassMgrs = new Collection<DeviceClass>();
                DeviceClassMgrs.Add(HidDeviceClass.Instance);
                DeviceClassMgrs.Add(SerialPortDeviceClass.Instance);
            }
        ...
        private static NativeMethods.SP_CLASSIMAGELIST_DATA _ClassImageList;

        /// <summary>
        /// Returns an HIMAGELIST from setupapi.dll
        /// </summary>
        internal static NativeMethods.SP_CLASSIMAGELIST_DATA ClassImageList
        {
            get
            {
                lock (_TheLock)
                {
                    ...
                    {
                        _ClassImageList = new NativeMethods.SP_CLASSIMAGELIST_DATA();

                        if (!NativeMethods.SetupDiGetClassImageList(_ClassImageList))
                        {
        ...
        public static void AttachImageList(Control ctrl)
        {
            ...
            if (ctrl is TreeView)
            {
                msg = NativeMethods.TVM_SETIMAGELIST;
            }
            else if (ctrl is ToolBar || ctrl is ToolStrip)
            {
                msg = NativeMethods.TB_SETIMAGELIST;
            }
            ...
            {
                msg = NativeMethods.CBEM_SETIMAGELIST;
            }
            else if (ctrl is TabControl)
            {
                msg = NativeMethods.TCM_SETIMAGELIST;
            }
            else
            {
            ...
            lock (_TheLock)
            {
                ...
                IntPtr retVal = NativeMethods.SendMessage(hCtrl, msg, new IntPtr(0), ClassImageList.ImageList);
            }
        }
...
//                    var info = new NativeMethods.IMAGEINFO();
                    int imageCount = NativeMethods.ImageList_GetImageCount(ClassImageList.ImageList);
                    for (int i = 0; i < imageCount; ++i)
                    {
//                        bool ret = NativeMethods.ImageList_GetImageInfo(ClassImageList.ImageList, i, ref info);
//                        UInt32 backgroundColor = NativeMethods.ImageList_GetBkColor(ClassImageList.ImageList);
//                        UInt32 oldColor = NativeMethods.ImageList_SetBkColor(ClassImageList.ImageList, NativeMethods.CLR_NONE);
                        IntPtr hIcon = NativeMethods.ImageList_GetIcon(ClassImageList.ImageList, i, NativeMethods.ILD_NORMAL);
                        System.Drawing.Bitmap bitmap = System.Drawing.Icon.FromHandle(hIcon).ToBitmap();
//                        bitmap.Save(String.Format("{0}.bmp", i));
        ...
        #region NativeMethods.IHWEventHandler Implemenation

        int NativeMethods.IHWEventHandler.Initialize(String pszParams)
        {
            return NativeMethods.S_OK;
        }

        int NativeMethods.IHWEventHandler.HandleEventWithContent(
...
//                    String msg = String.Format("Rejected \"{0}\" AutoPlay event for device: {1}( {2} )", pszEventType, pszAltDeviceID, pszDeviceID);
        ...
        #endregion // NativeMethods.IHWEventHandler Implemenation

        #region NativeMethods.IQueryCancelAutoPlay Implementation
...
//                        String msg = String.Format("Rejected AutoPlay for drive {0}", pszPath);
        ...
        #endregion // NativeMethods.IQueryCancelAutoPlay Implementation
        ...
        private void Dispose(bool disposing)
        {
            ...
            if (!_Disposed)
            {
                ...
                if (_ClassImageList != null)
                {
                    ...
                    {
                        bool ret = NativeMethods.SetupDiDestroyClassImageList(_ClassImageList);
...
//                bool index = EventWaitHandle.WaitOne();//timeout * 1000, false);
                ...
                int ret;
                if ((ret = NativeMethods.MsgWaitForMultipleObjectsEx(1, handles, timeout * 1000, 0, 0)) == 0)
                {
                    // Timeout
                    break;
                }
...
*/
        public Device FindDevice(Type devClass, UInt16? vid, UInt16? pid)
        {
            ...
            if (devClass != null)
            {
                ...
                else if (devClass.IsAssignableFrom(typeof(SerialPortDeviceClass)) )
                    deviceClass = SerialPortDeviceClass.Instance;
            ...
            {
                foreach (DeviceClass dev_class in DeviceClassMgrs)
                {
...
//            Trace.WriteLine(String.Format("UsbPort.FindChildDevice() - DevInst:{0}, Path:{1}", tempDev.DeviceInfoData.devInst, tempDev.Path));
            ...
            Device dev;
            if (NativeMethods.GUID_DEVCLASS_HIDCLASS == tempDev.ClassGuid)
            {
...
//                Trace.WriteLine(String.Format(" HidDevice - DevInst:{0}, Path:{1}", dev.DeviceInstance, dev.Path));
            ...
            else if (NativeMethods.GUID_DEVCLASS_PORTS == tempDev.ClassGuid)
            {
                SerialPortDeviceClass devMgr = SerialPortDeviceClass.Instance;
                dev = devMgr.FindDeviceByDriver(tempDev.Driver);// CreateDevice(tempDev.DeviceInfoData, tempDev.Path);
...
//                Trace.WriteLine(String.Format(" Device - DevInst:{0}, Path:{1}", dev.DeviceInstance, dev.Path));
        ...
        public static UInt32 FindDeviceInstance(String driverToFind)
        {
            UInt32 devInst;
            UInt32 devInstNext;
            NativeMethods.CONFIGRET cr;
            bool walkDone = false;
            int len;

            // Get Root DevNode
            //
            cr = NativeMethods.CM_Locate_DevNode(out devInst, null, 0);
            ...
            while (!walkDone)
            {
                ...
                len = buf.Capacity;
                cr = NativeMethods.CM_Get_DevNode_Registry_Property(
                    devInst,
                    NativeMethods.CM_DRP_DRIVER,
                    IntPtr.Zero,
                    buf,
                    ref len,
                    0);
                ...
                //
                if (cr == NativeMethods.CONFIGRET.CR_SUCCESS && String.Compare(buf.ToString(), driverToFind, true) == 0)
                {
                    // SUCCESS
                    return devInst;
                }
                ...
                //
                cr = NativeMethods.CM_Get_Child( out devInstNext, devInst, 0);
                ...
                for (; ; )
                {
                    cr = NativeMethods.CM_Get_Sibling( out devInstNext, devInst, 0);
                    ...
                    cr = NativeMethods.CM_Get_Parent( out devInstNext, devInst, 0);
